# Comportamiento y Personalidad del Asistente WEB7 Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Actualizar el prompt de sistema del asistente en el backend para humanizar el tono en español rioplatense y guiar una conversación calificada de leads (nombre, negocio, email).

**Architecture:** Modificación del prompt del sistema en la API Edge de Vercel (`api/chat.js`) para inyectar las nuevas directivas conversacionales estructuradas en fases, el anclaje de precios de landing pages, y la solicitud orgánica de datos.

**Tech Stack:** Javascript, Vercel Serverless (Edge Runtime), Groq SDK (Llama 3.3 70b).

---

### Task 1: Modificación de la directiva de sistema (SYSTEM_PROMPT)

**Files:**
- Modify: `api/chat.js`

- [ ] **Step 1: Modificar el archivo `api/chat.js` con el nuevo SYSTEM_PROMPT**

Escribir el código en [api/chat.js](file:///Users/carlosfedericogunther/Downloads/Claudio/studio-demo/api/chat.js) reemplazando la constante `SYSTEM_PROMPT` con la siguiente definición:
...
- [ ] **Step 2: Validar la sintaxis de `api/chat.js`**

Ejecutar: `node --check api/chat.js`
Esperado: Salida vacía (sin errores sintácticos).

- [ ] **Step 3: Realizar commit de los cambios locales**

Ejecutar:
```bash
git add api/chat.js
git commit -m "feat: rediseñar el prompt del asistente de chat para un tono mas humano y flujo conversacional"
```

- [ ] **Step 4: Desplegar a producción en Vercel**

Ejecutar: `npx vercel --prod`
Esperado: Aliased: `https://web7-studio.vercel.app`.
